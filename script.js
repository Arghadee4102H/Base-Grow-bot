...
// Loader Fail-safe (Show retry button if stuck more than 10 seconds)
setTimeout(() => {
    if (document.getElementById('app').classList.contains('hidden')) {
        document.getElementById('loader-msg').innerText = "Connection taking too long...";
...
onAuthStateChanged(auth, async (user) => {
    ...
    setTimeout(() => { document.getElementById('loader-screen').classList.add('hidden'); }, 500);
...
function initApp() {
    ...
    onSnapshot(query(collection(db, "tasks"), where("budget", ">", 0), limit(20)), (snap) => {
        ...
        snap.forEach(tDoc => {
            ...
            div.innerHTML = `<span>Follow Profile</span><button class="btn-neon" style="width:80px" onclick="doTask('${tDoc.id}','${t.link}')">1 PT</button>`;
...
document.getElementById('watch-ad-btn').onclick = () => {
    ...
    else { alert("Ad simulated"); setTimeout(rewardAd, 2000); }
};
...
document.getElementById('submit-campaign').onclick = async () => {
    ...
    if(userLocal.points < cost || userLocal.points < 21) return alert("Insufficient points (Min 21)");
